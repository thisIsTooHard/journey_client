...
namespace data
{
	...
	vector2d footholdtree::getpfedges(short id)
	{
		vector2d ret = vector2d();

		if (footholds.count(id))
		{
			foothold start = footholds[id];
			...
			while (prev != 0)
			{
				...
				if (prev == 0)
				{
					ret.setx(start.horizontal.x());
				}
				else
				{
					start = footholds[prev];
				}
			}
			...
			while (next != 0)
			{
				...
				if (next == 0)
				{
					ret.sety(start.horizontal.y());
				}
				else
				{
					start = footholds[next];
				}
			}
		}
