...
namespace io
{
	charcreation::charcreation(bool fem)
	{
		...
		nl::node bgsrc = nl::nx::nodes["Back"]["Back"]["login.img"]["back"];
		...
		textfields[TXT_NAMECHAR] = textfield(app.getfonts()->getfont(DWF_LEFT), txc_white, "", vector2d(490, 217), 12);
		textfields[TXT_NAMECHAR].setstate("active");
		app.getui()->settextfield(&textfields[TXT_NAMECHAR]);

		setlabel = textlabel(app.getfonts()->getfont(dwf_small_c), txc_black, "");
	...
	void charcreation::draw(ID2D1HwndRenderTarget* target)
	{
		...
		if (active)
		{
			...
			{
				setlabel.draw(newchar.getface()->getname(), target, vector2d(589, 214));
				setlabel.draw(newchar.gethair()->getname(), target, vector2d(589, 233));
				setlabel.draw(newchar.gethair()->getcolor(), target, vector2d(589, 252));
				setlabel.draw(newchar.getbody()->getname(), target, vector2d(589, 271));
				setlabel.draw(newchar.getcloth("Coat")->getname(), target, vector2d(589, 290));
				setlabel.draw(newchar.getcloth("Pants")->getname(), target, vector2d(589, 309));
				setlabel.draw(newchar.getcloth("Shoes")->getname(), target, vector2d(589, 328));
				setlabel.draw(newchar.getcloth("Weapon")->getname(), target, vector2d(589, 347));
				setlabel.draw(female? "Female" : "Male", target, vector2d(589, 366));
			}
		}
	}
