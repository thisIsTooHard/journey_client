...
namespace gameplay
{
	...
	const maplestat statvalues[20] = { SKIN, FACE, HAIR, LEVEL, JOB, STR, DEX, INT_, LUK, HP, MAXHP, MP, MAXMP, AP, SP, EXP, FAME, MESO, PET, GACHAEXP };
	...
	{
	public:
		maplestats(int, string, bool, short, int, int, int, short, map<maplestat, short>, pair<int, char>, vector<int64_t>);
		maplestats() {}
		~maplestats() {}
		...
		void setmaxhp(int m) { totalmaxhp = m; }
		void setmaxmp(int m) { totalmaxmp = m; }
		float getmastery() { return mastery; }
		float getcritical() { return critical; }
		int getmaxhp() { return totalmaxhp; }
		int getmaxmp() { return totalmaxmp; }
