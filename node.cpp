...
namespace nl {
	...
	int8_t node::toi8(int8_t def) const {
		return istype(integernode) ? static_cast<int8_t>(m_data->ireal) : def;
	}
	int16_t node::toi16() const {
		return istype(integernode) ? static_cast<int16_t>(m_data->ireal) : 0;
	}
	int16_t node::toi16(int16_t def) const {
		return istype(integernode) ? static_cast<int16_t>(m_data->ireal) : def;
	}
	int32_t node::toi32() const {
		return istype(integernode) ? static_cast<int32_t>(m_data->ireal) : 0;
	}
	int32_t node::toi32(int32_t def) const {
		return istype(integernode) ? static_cast<int32_t>(m_data->ireal) : def;
	}
	int64_t node::toi64() const {
		return istype(integernode) ? m_data->ireal : 0;
	}
	int64_t node::toi64(int64_t def) const {
		return istype(integernode) ? m_data->ireal : def;
	}
	vector2d node::tov2d() const {
		return istype(vectornode) ? vector2d(m_data->vector[0], m_data->vector[1]) : vector2d();
	}
