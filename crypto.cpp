...
namespace net
{
	...
	void crypto::mapledecrypt(char* bytes, int length)
	{
		for (int j = 1; j <= 6; j++) {
			byte remember = 0;
			byte dataLength = static_cast<byte>(length & 0xFF);
			byte nextRemember;
			if (j % 2 == 0) {
				for (int i = 0; i < length; i++) {
					byte cur = bytes[i];
					cur -= 0x48;
					cur = static_cast<byte>((~cur) & 0xFF);
	...
	void crypto::aescrypt(char* bytes, int length, bool recv)
	{
		...
		int start = 0;
		while (length > 0) {
			vector<byte> miv = (recv) ? recviv : sendiv;
			for (int i = 0; i < 12; i++)
			{
	...
	void crypto::updateiv(bool recv)
	{
		if (MAPLECRYPTO)
		{
			byte mbytes[4] = { 0xf2, 0x53, 0x50, 0xc6 };
			...
			for (char i = 0; i < 4; i++) 
			{
				...
				mbytes[0] = static_cast<byte>(ret_value & 0xFF);
				mbytes[1] = static_cast<byte>((ret_value >> 8) & 0xFF);
				mbytes[2] = static_cast<byte>((ret_value >> 16) & 0xFF);
				mbytes[3] = static_cast<byte>((ret_value >> 24) & 0xFF);
			}
		...
		else
		{
			...
			for (char i = 0; i < 4; i++) 
			{
				...
				char cval = static_cast<char>(value);
				jrbytes[i] = (cval < 0) ? cval + 128 : cval;
				newiv.push_back(jrbytes[i]);
			}
			sendiv = newiv;
		}
	}
